...
void RGBmatrixPanel::Write_REG1(int A,int B,int C,int D,int E,int F,uint16_t REG_DATA)
{
...
void RGBmatrixPanel::Write_REG2(int A,int B,int C,int D,int E,int F,uint16_t REG_DATA)
{
  digitalWrite(CLK, LOW);
  digitalWrite(LAT, LOW);
...
void RGBmatrixPanel::begin(void)
{
  ...
  *addrcport &= ~addrcpin; // Low
  ...
  // 保存原始 DDRA 的低 2 位状态
  uint8_t ddraSave = DDRA & B00000011;
  
  // 只设置高6位为输出，保护 PORTA 的低2位
  DATADIR  = B11111100;
  DATAPORT = 0;
  
  // 恢复低 2 位（如果原来是输入）
  DDRA = (DDRA & B11111100) | ddraSave;
...
// 8/8/8 -> gamma -> 5/6/5
uint16_t RGBmatrixPanel::Color888(uint8_t r, uint8_t g, uint8_t b, boolean gflag)
{
...
uint16_t RGBmatrixPanel::ColorHSV(long hue, uint8_t sat, uint8_t val, boolean gflag)
{
...
void RGBmatrixPanel::customizeZH(const uint8_t *arr, uint8_t fontSize, uint8_t x, uint8_t y, uint16_t color)
{
